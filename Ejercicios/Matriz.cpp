...
template<typename T>
Matriz<T> Matriz<T>::operator* (const Matriz &matrix_2)
{
    Matriz matrix_result( fila, matrix_2.columna );
    T total;
 
    for (int i = 0; i < fila; i++) {
        for (int j = 0; j < matrix_2.columna; j++) {
            for (int k = 0; k < columna; k++) {
                total += (M[i][k] * matrix_2.M[k][j]);
            }
            matrix_result.M[i][j] = total;
